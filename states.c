...
void state2(int *symbol, int *state, int *movement){
	...
	if(*symbol ==2){
		...
		*movement= -1; 
		*state=ACCEPT; 
		fprintf(stdout,"It accepts because n doesn't divide m\n");
	}
...
void state6(int *symbol, int *state, int *movement){
	if(*symbol ==5){
		*symbol=5; 
		*movement=0; // Only because it accepts. And it accepts because the first time that n fits in m, it reaches the end of m, so n=m.
		*state=ACCEPT; 
		fprintf(stdout, "\nIt accepts because the first time that n fits in m, it reaches the end of m, so n=m. \n");
	}

	else if(*symbol ==6){
		*symbol=6; // Lo cambiamos en anticipación de la siguiente función.
